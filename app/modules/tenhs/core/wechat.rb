...
module Tenhs
  module Core
    module Wechat
      ...
      def wechat_auth(config, scope = nil)
        ...
        if session[:openid].blank?
          redirect_to Tenhs::Core::WechatService.auth_url(request.original_url, config, scope)
        end
      end

      ## 使用sns_userinfo时获取用户数据
      def wechat_userinfo(config)
        Tenhs::Core::WechatService.get_user_info(session[:openid], session[:access_token], config)
      end
