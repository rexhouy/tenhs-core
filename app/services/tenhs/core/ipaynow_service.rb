...
class Tenhs::Core::IpaynowService
  ...
  ## config = {web_id: "", web_key: "", appid: "公众号appid，按方式2时需要传递"}
  def self.to_pay(pay_param, config, openid = nil)
    params = {
    ...
    params[:mhtSignature] = sign(params, config)
    if openid.present?
  ...
  ## 检查参数是否正确
  def self.verify(params, config)
    sig = params["signature"]
    params.delete("signature")
    sig.eql?(sign(params, config))
  end

  private

  def self.sign(params, config)
